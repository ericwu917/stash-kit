...
个人工具集仓库，通过 GitHub Pages + jsdelivr CDN 静态托管。包含三类内容：
...
3. **Claude Code 状态栏** (`claude/statusline.sh`) — 自定义终端状态栏，显示模型、上下文窗口、速率限制等信息

## 技术栈

- 纯静态文件，无构建系统、无包管理器、无测试框架
- JavaScript (ES6)：Stash 代理脚本和 Web UI
- Bash + jq：CLI 工具和 Claude Code 状态栏
...
```
https://fastly.jsdelivr.net/gh/taige/taige.github.io@master/path/to/file
```
...
- **状态栏脚本**：`claude/statusline.sh` 从 stdin 读取 Claude Code 提供的 JSON，输出两行 ANSI 彩色文本。依赖 bash、jq、git。
